...
export default function ChatWindow() {
  ...
  useEffect(() => { dispatch(loadOlder()); }, [dispatch]);
  ...
  return (
    <Box>
      ...
      <Box ref={containerRef} sx={{ height: 400, overflowY: 'auto', p:2, bgcolor:'#f5f5f5' }}>
        {items.map(msg => (
          <MessageBubble key={msg.id} message={msg} />
        ))}
        {typing && (
          <Box sx={{ fontStyle:'italic', color:'#777', m:1 }}>
      ...
      <Box sx={{ display:'flex', gap:1, mt:2 }}>
        <TextField inputRef={inputRef} placeholder="Type a message..." fullWidth onKeyDown={(e) => e.key === 'Enter' && handleSendText()} />
        <ImageUpload onUpload={(url:any) => handleSend(undefined, url)} />
